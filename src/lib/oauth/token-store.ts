...
// Layout: a single JSON file keyed by API base-URL ORIGIN:   { "<origin>": { access_token,
// refresh_token, expires_at, scope } } at $XDG_CONFIG_HOME/framedash/credentials.json,
// defaulting to ~/.config/framedash/credentials.json. Windows deliberately uses the same
// ~/.config path (documented in the CLI README) so one path rule covers every platform.
//
// CONCURRENCY MODEL: atomic-write, last-writer-wins. Every write goes to a temp file in the
// same directory and is renamed over the destination, so readers always see an internally
// consistent snapshot; there is NO cross-process locking. Two concurrent same-origin refreshes
// can therefore race rotation, whose worst case is the server's reuse revocation -- which
// surfaces as the ordinary invalid_grant "run framedash login again" path. That trade (a rare
// interactive re-login vs a hand-rolled lock subsystem) is deliberate; CI uses API keys and is
// unaffected.
//
// File/dir modes are 0700/0600 on POSIX; chmod is a silent no-op on Windows. A corrupt or
// unreadable store is treated as absent (never crashes the CLI). Token values must NEVER be
// logged; callers may print scope/expiry only.
...
/**
 * Missing, unreadable, or corrupt files -- and individual malformed entries -- degrade to
 * "absent" rather than throwing: the caller then falls back to other credential sources or asks
...
/**
 * Temporary files from an interrupted credential write hold the same token material as the
 * store, so logout --all must remove them too.
 */
